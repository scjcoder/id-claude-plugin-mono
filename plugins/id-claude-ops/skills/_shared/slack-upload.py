...
Fetches the bot token from AWS Secrets Manager and uploads a file to a Slack DM.

IMPORTANT: Run this via Desktop Commander (mcp__Desktop_Commander__start_process),
NOT via the sandbox bash tool. The sandbox has no AWS credentials or network access
to Slack. Desktop Commander runs directly on the user's Mac where AWS CLI is configured.
...
Options:
  --user    Slack user ID to open a DM with (default: U068C9UNX6U = Sean Johnson)
...
SEAN_USER_ID = "U068C9UNX6U"
AWS_PROFILE  = "Install-982534385600"
...
def get_token() -> str:
    ...
    return json.loads(r.stdout)["slack"]["bot_token"]
