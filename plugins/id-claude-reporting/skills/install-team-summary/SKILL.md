...
If zero active items are found across both sources, send a short Slack message via the slack_send_message MCP tool to channel `<SLACK_DM_SEAN>` instead: `"*Install Team Summary* · {today} — No active items in the past N days."` (substitute actual N).
