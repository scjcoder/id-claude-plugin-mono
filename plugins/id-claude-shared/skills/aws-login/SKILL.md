...
name: aws-login
description: >
  Log in to the InsideDesk AWS account (982534385600) using SSO via the
...
`Install-982534385600` profile are already set up on the user's Mac.
...
| Profile name | `Install-982534385600` |
...
```
aws sts get-caller-identity --profile Install-982534385600
```

- Returns JSON with `"Account": "982534385600"` → already authenticated, skip to Step 3.
...
Use Desktop Commander (`start_process`) to run:
```
aws sso login --profile Install-982534385600
```

This opens the user's browser to `https://seanjo.awsapps.com/start/#/device`
and shows a device code. Wait for:

```
Successfully logged into Start URL: https://seanjo.awsapps.com/start
```
...
{
  "UserId": "...",
  "Account": "982534385600",
  "Arn": "arn:aws:sts::982534385600:assumed-role/AWSReservedSSO_install_.../..."
}
```
...
```
--profile Install-982534385600 --region us-east-1
```
...
- **Login times out:** Re-run `aws sso login --profile Install-982534385600` via Desktop Commander.
